...
// ------------------------------------------------------------
// Turns a project title into a safe, readable filename base,
// e.g. "The Ancient Oak of Alderbrook" -> "the-ancient-oak-of-alderbrook"
// Falls back to a short date-based name if the title is unusable.
// ------------------------------------------------------------
function slugify(title) {
  const slug = (title || '')
    ...
    .slice(0, 50)
    .replace(/-+$/, '');
  return slug || `b8ai-project-${Date.now()}`;
}

// Builds the actual filenames for one project, once per request —
// dynamic and content-based instead of always "storyboard.pdf" etc.
function buildFileNames(project) {
  const base = slugify(project.title);
  const isContent = !!project.content;
  return {
    pdf:  isContent ? `${base}.pdf`            : `${base}-storyboard.pdf`,
    docx: isContent ? `${base}.docx`           : `${base}-shotlist.docx`,
    pptx: `${base}-presentation.pptx`,
    xlsx: `${base}-tracker.xlsx`,
    srt:  `${base}-subtitles.srt`,
    txt:  isContent ? `${base}.txt`            : `${base}-script.txt`,
    md:   isContent ? `${base}.md`             : `${base}-README.md`,
...
async function generatePPTX(project) {
  ...
  // Simple, consistent template theme
  const ACCENT = '5B4FE9';
  const DARK = '1A1A1E';
  const LIGHT_GRAY = '8A8A94';

  pptx.defineSlideMaster({
    title: 'B8AI_MASTER',
    background: { color: 'FFFFFF' },
    objects: [
      { rect: { x: 0, y: 0, w: '100%', h: 0.12, fill: { color: ACCENT } } },
    ],
  });

  // Title slide — dark background, big centered title, accent underline
  const titleSlide = pptx.addSlide();
  titleSlide.background = { color: DARK };
  titleSlide.addShape(pptx.ShapeType.rect, { x: 0.6, y: 2.55, w: 1.4, h: 0.06, fill: { color: ACCENT } });
  titleSlide.addText(project.title || 'B8AI Director Project', {
    x: 0.5, y: 1.9, w: 9, fontSize: 32, bold: true, color: 'FFFFFF', fontFace: 'Helvetica'
  });
  titleSlide.addText('Generated by B8AI Director', {
    x: 0.5, y: 4.6, w: 9, fontSize: 12, color: LIGHT_GRAY, fontFace: 'Helvetica'
  });

  const addSectionSlide = (heading, bodyLines) => {
    const slide = pptx.addSlide({ masterName: 'B8AI_MASTER' });
    if (heading) {
      slide.addText(heading, { x: 0.5, y: 0.4, w: 9, fontSize: 24, bold: true, color: DARK, fontFace: 'Helvetica' });
      slide.addShape(pptx.ShapeType.rect, { x: 0.5, y: 1.05, w: 1.2, h: 0.04, fill: { color: ACCENT } });
    }
    slide.addText(
      bodyLines.map(line => ({ text: line, options: { bullet: bodyLines.length > 1, breakLine: true } })),
      { x: 0.5, y: heading ? 1.3 : 0.5, w: 9, h: 4.5, fontSize: 14, color: '333333', fontFace: 'Helvetica', valign: 'top' }
    );
  };
  ...
  if (project.content) {
    ...
    sections.forEach((section) => addSectionSlide(section.heading, section.lines));
  } else {
    (project.scenes || []).forEach((scene, i) => {
      const lines = [scene.description || ''];
      if (scene.effects?.length) lines.push(`Effects: ${scene.effects.join(', ')}`);
      addSectionSlide(`Scene ${i + 1}: ${scene.title || ''}`, lines);
    });
  }
...
router.post('/v1/generate-files', async (req, res) => {
  try {
    ...
    const names = buildFileNames(project);
    const generators = {
      pdf:  { name: names.pdf,  fn: generatePDF },
      docx: { name: names.docx, fn: generateDOCX },
      pptx: { name: names.pptx, fn: generatePPTX },
      xlsx: { name: names.xlsx, fn: generateXLSX },
      srt:  { name: names.srt,  fn: async (p) => generateSRT(p) },
      txt:  { name: names.txt,  fn: async (p) => generateScriptTXT(p) },
      md:   { name: names.md,   fn: async (p) => generateReadmeMD(p) },
      json: { name: names.json, fn: async (p) => generateProjectJSON(p) },
      csv:  { name: names.csv,  fn: async (p) => generateSceneCSV(p) },
    };
...
router.post('/v1/generate-file/:format', async (req, res) => {
  ...
  const names = buildFileNames(project);
  const single = {
    pdf:  { name: names.pdf,  mime: 'application/pdf', fn: generatePDF },
    docx: { name: names.docx, mime: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', fn: generateDOCX },
    pptx: { name: names.pptx, mime: 'application/vnd.openxmlformats-officedocument.presentationml.presentation', fn: generatePPTX },
    xlsx: { name: names.xlsx, mime: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', fn: generateXLSX },
    srt:  { name: names.srt,  mime: 'text/plain', fn: async (p) => generateSRT(p) },
    txt:  { name: names.txt,  mime: 'text/plain', fn: async (p) => generateScriptTXT(p) },
    md:   { name: names.md,   mime: 'text/markdown', fn: async (p) => generateReadmeMD(p) },
    json: { name: names.json, mime: 'application/json', fn: async (p) => generateProjectJSON(p) },
    csv:  { name: names.csv,  mime: 'text/csv', fn: async (p) => generateSceneCSV(p) },
  }[format];
